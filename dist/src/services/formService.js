...
export class FormsService {
    async submitForm(submissionData, metadata) {
        try {
    ...
    async getFormSubmissions(formId, userEmail, limit = 10, offset = 0) {
        try {
            const filter = {};
            if (formId)
    ...
    async checkSubmissionProtection(sessionId, ipAddress) {
        const now = new Date();
        ...
        if (recentSubmissions.length > 0) {
            ...
            await FormSubmissionModel.updateOne({ _id: lastSubmission._id }, {
    ...
    async cleanupConvertedDraft(sessionId) {
        try {
